...
namespace PanoramicData.utils
{
    ...
    class Convexhull
    {
        public static IList<Vector2> convexhull(IList<Point> pointList)
        {
    ...
    class TestCH
    {
        ...
        static void MainX(string[] args)
        {
            ...
            if (args.Length == 1)
            {
                ...
                for (int i = 0; i < N; i++)
                    pts[i] = XPoint.Random(500, 500);
                XPoint[] chpts = Convexhull.convexhull(pts);
                Debug.WriteLine("Area is " + area(chpts));
                print(chpts);
            }
            else
                Debug.WriteLine("Usage: TestCH <XPointcount>\n");
             */
        }

        // The centroid of a XPoint set
        public static XPoint centroid(XPoint[] pts)
        {
            int N = pts.Length;
            ...
            return new XPoint(sumx / N, sumy / N);
        }

        // The area of a polygon (represented by an array of ordered vertices)
        public static double area(XPoint[] pts)
        {
            int N = pts.Length;
            XPoint centr = centroid(pts);
            double area2 = 0;
            for (int i = 0; i < N; i++)
                area2 += XPoint.Area2(centr, pts[i], pts[(i + 1) % N]);
            return Math.Abs(area2 / 2);
        }

        public static void print(XPoint[] pts)
        {
            int N = pts.Length;
            for (int i = 0; i < N; i++)
