...
namespace Microsoft.Research.DynamicDataDisplay.Charts
{
	internal static class RoundHelper
	{
        ...
        internal static double Round(double number, int rem) {
			if (rem <= 0)
			{
                //bcz: added conditional statement
                if (number == .25 || Math.Abs(number - .3333333333) < 0.0000001 || number == 1 / 6.0 || number == 1/8.0
                    || number == 1/16.0 || number == 1/32.0 || number == 1/64.0 || number == 1/128.0 || number == 1/256.0)
                    return number;
			...
			else
			{
				...
				double val = pow * Math.Round(number / pow);
				return val;
			}
		}
