	...
	public abstract class AxisBase<T> : ContentControl, ITypedAxis<T>, IValueConversion<T>
	{
		...
		private void OnViewportPropertyChanged(object sender, ExtendedPropertyChangedEventArgs e)
		{
			...
			using (axisControl.OpenUpdateRegion())
			{
                ...
                if (axisControl.AxisTiedToOrigin) {  //bcz: added conditional and contents
                    if (axisControl.Placement == AxisPlacement.Left || axisControl.Placement == AxisPlacement.Right) {
                        double min = visible.Left;
                        double max = visible.Right;
                        if (max > 0 && min < 0) {
                            axisControl.Placement = AxisPlacement.Left;
                        ...
                        if (max < 0) {
                            axisControl.Placement = AxisPlacement.Right;
                            axisControl.RenderTransform = new TranslateTransform(viewport.Output.Width+axisControl.ActualWidth, 0);
                        }
                    }
                    if (axisControl.Placement == AxisPlacement.Bottom || axisControl.Placement == AxisPlacement.Top) {
                        double min = visible.Bottom;
                        double max = visible.Top;
                        if (max < 0 && min > 0) {
                            axisControl.Placement = AxisPlacement.Bottom;
                        ...
                        if (min < 0) {
                            axisControl.Placement = AxisPlacement.Top;
                            axisControl.RenderTransform = new TranslateTransform(0, -viewport.Output.Height - axisControl.ActualHeight);
                        }
                    }
                }
			}
