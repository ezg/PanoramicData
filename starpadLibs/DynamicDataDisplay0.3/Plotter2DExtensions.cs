...
namespace Microsoft.Research.DynamicDataDisplay
{
	...
	public static class Plotter2DExtensions
	{
        ...
        // bcz: added
        public static void RemoveLineGraph(this Plotter2D plotter, LineAndMarker<MarkerPointsGraph> graph) {
            if (graph.MarkerGraph != null)
                plotter.Children.Remove(graph.MarkerGraph);
            if (graph.LineGraph != null)
		...
		public static LineGraph AddLineGraph(this Plotter2D plotter, IPointDataSource pointSource, Pen linePen, Description description)
		{
		    ...
		    graph.Description = description; // bcz: modified
			graph.Filters.Add(new InclinationFilter());
			graph.Filters.Add(new FrequencyFilter());
			//graph.Filters.Add(new CountFilter());
			plotter.Children.Add(graph);
