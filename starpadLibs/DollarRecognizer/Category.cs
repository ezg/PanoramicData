...
namespace starPadSDK.DollarRecognizer
{
    public class Category
    {
        ...
        public Category(string name, DGesture firstExample)
        {
            _name = name;
        ...
        public Category(string name, ArrayList examples)
        {
            ...
            {
                DGesture p = (DGesture) examples[i];
                AddExample(p);
            }
        }
        ...
        /// <param name="i"></param>
        /// <returns></returns>
        public DGesture this[int i]
        {
            get
            {
                if (0 <= i && i < _prototypes.Count)
                {
                    return (DGesture) _prototypes[i];
                }
                else
                {
                    return null;
        ...
        public void AddExample(DGesture p)
        {
            bool success = true;
            try
            {
                ...
                {
                    DGesture p0 = (DGesture) _prototypes[i];
                    if (p0.Name == p.Name)
