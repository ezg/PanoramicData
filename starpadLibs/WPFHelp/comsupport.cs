...
namespace starPadSDK.WPFHelp
{
	...
	public class ComSupport
	{
        ...
        #region raw device APIs

        [DllImport("user32.dll", SetLastError = true)]
        public static extern uint GetRawInputDeviceList(IntPtr pRawInputDeviceList, ref int puiNumDevices, int cbSize);
        ...
        [StructLayout(LayoutKind.Sequential)]
        public struct RID_DEVICE_INFOheader
        {
            public int cbSize;
            public int dwType;
        }
        ...
        public int GET_RAWINPUT_CODE_WPARAM(IntPtr wParam) {
            return wParam.ToInt32() & 0xff;
        }

        /// <summary>
        /// "The input is in the regular message flow, the app is required to call DefWindowProc so that the system can perform clean ups."
        /// </summary>
        public const int RIM_INPUT = 0;
        /// <summary>
        /// "The input is sink only. The app is expected to behave nicely."
        /// </summary>
        ...
        public static ComSupport.RAWINPUTDEVICELIST[] GetRawInputDeviceList() {
            int ndev = 0;
            int dwSize = Marshal.SizeOf(typeof(ComSupport.RAWINPUTDEVICELIST));
            uint val = ComSupport.GetRawInputDeviceList(IntPtr.Zero, ref ndev, dwSize);
            //Trace.Assert(val == 0);
            ComSupport.RAWINPUTDEVICELIST[] ridl = new ComSupport.RAWINPUTDEVICELIST[ndev];
            IntPtr pridl = Marshal.AllocHGlobal((int)(dwSize * ndev));
            val = ComSupport.GetRawInputDeviceList(pridl, ref ndev, dwSize);
            //Trace.Assert(val == ndev);
            for (int i = 0; i < ndev; i++)
                ridl[i] = (ComSupport.RAWINPUTDEVICELIST)Marshal.PtrToStructure(new IntPtr(pridl.ToInt32() + dwSize * i), typeof(ComSupport.RAWINPUTDEVICELIST));
            Marshal.FreeHGlobal(pridl);
            return ridl;
        }

        public static ComSupport.RID_DEVICE_INFOheader GetDeviceInfo(ComSupport.RAWINPUTDEVICELIST r, out IntPtr devinfo) {
            uint pcbSize = 0;
            uint val = ComSupport.GetRawInputDeviceInfo(r.hDevice, ComSupport.RIDI_DEVICEINFO, IntPtr.Zero, ref pcbSize);
            devinfo = Marshal.AllocHGlobal((int)pcbSize);
            Marshal.WriteInt32(devinfo, (int)pcbSize);
            val = ComSupport.GetRawInputDeviceInfo(r.hDevice, ComSupport.RIDI_DEVICEINFO, devinfo, ref pcbSize);
            return (ComSupport.RID_DEVICE_INFOheader)Marshal.PtrToStructure(devinfo, typeof(ComSupport.RID_DEVICE_INFOheader));
        }

        public static string GetDeviceName(ComSupport.RAWINPUTDEVICELIST r) {
            uint pcbSize;
            pcbSize = 0;
            uint val = ComSupport.GetRawInputDeviceInfo(r.hDevice, ComSupport.RIDI_DEVICENAME, IntPtr.Zero, ref pcbSize);
            IntPtr pstring = Marshal.AllocHGlobal((int)pcbSize * 2);
            val = ComSupport.GetRawInputDeviceInfo(r.hDevice, ComSupport.RIDI_DEVICENAME, pstring, ref pcbSize);
            string name = Marshal.PtrToStringUni(pstring, (int)pcbSize);
            Marshal.FreeHGlobal(pstring);
            if (name[name.Length - 1] == 0)
                name = name.Substring(0, name.Length - 1);
            return name;
        }
        public static ComSupport.RAWINPUTHEADER GetRawInputHeader(IntPtr lParam, uint dwSize, IntPtr lpb) {
            uint actualsz = ComSupport.GetRawInputData(lParam, ComSupport.RID_INPUT, lpb, ref dwSize, (uint)Marshal.SizeOf(typeof(ComSupport.RAWINPUTHEADER)));
            //Trace.Assert(actualsz == dwSize);
            ComSupport.RAWINPUTHEADER rih = (ComSupport.RAWINPUTHEADER)Marshal.PtrToStructure(lpb, typeof(ComSupport.RAWINPUTHEADER));
            if (rih.dwType != ComSupport.RIM_TYPEHID)
                Console.WriteLine("Hm, got WM_INPUT from non-hid device: " + rih.dwType);
            return rih;
        }
        #endregion
        ...
        // Error constants
        public const int ERROR_INVALID_WINDOW_HANDLE = 0x578;
		// GetWindow constants
        ...
        [DllImport("user32.dll")]
        public static extern Boolean GetCursorPos(out POINT pt);
        [DllImport("user32.dll")]
        ...
        [DllImport("gdi32.dll")]
        public static extern int SetPixel(IntPtr hDC, int x, int y, int value);
        [DllImport("gdi32.dll")]
        public static extern int SetBkColor(IntPtr hDC, int x);

        [DllImport("user32.dll")]
        public static extern Boolean DrawIconEx(IntPtr hdc, int xLeft, int yTop, IntPtr hIcon, int cxWidth, int cyHieght, uint indexOfFrameInAnimatedCursor, IntPtr handleToBackgroundBrush, uint drawFlags);
